...
async def currency(self: Client, ctx: Message):
    ...
    if len(ctx.text.split()) != 4:
        return await ctx.reply(f"Use format /{ctx.command[0]} [amount] [currency_from] [currency_to] to convert currency.", del_in=6)
    teks = ctx.text.split()
    amount = teks[1]
    ...
    if amount.isdigit() or (amount.replace('.','',1).isdigit() and amount.count('.') < 2):
        ...
        try:
            ...
            await ctx.reply(f"**CURRENCY EXCHANGE RATE RESULT:**\n\n`{format(float(amount), ',')}` **{base_code}** = `{format(float(conversion_result), ',')}` **{target_code}**\n<b>Rate Today</b> = `{format(float(conversion_rate), ',')}`\n<b>Last Update:</b> {last_update}")
        except Exception as err:
            await ctx.reply(f"Failed convert currency, maybe you give wrong currency format or api down.\n\n<b>ERROR</b>: {err}")
    else:
        await ctx.reply("<code>This seems to be some alien currency, which I can't convert right now.. (⊙_⊙;)</code>")
